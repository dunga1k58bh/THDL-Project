...
function Home() {
  ...
  const selectedDisplaySize = useSelector(state => state.product.selectedDisplaySize);
  ...
  useEffect(() => {
    axios.post("http://localhost:3001/phone").then((res) => {
      setData(res.data.result.data);
      setTotalPage(res.data.result.pagination.total)
    });
    axios.get("http://localhost:3001/filter").then((res) => {
      ...
      if (selectedDisplaySize.length !== data.display_size?.length) {
        dispatch(updateState({selectedDisplaySize: data.rom?.map(() => false)}));
      }
      if (selectedBattery.length !== data.battery?.length) {
        dispatch(updateState({selectedBattery: data.battery?.map(() => false)}));
      }
      if (selectedCpuType.length !== data.cpu_type?.length) {
        dispatch(updateState({selectedCpuType: data.cpu_type?.map(() => false)}));
      }
    });
  }, []);
  const search = () => {
    const filter = {
      ...
      display_size: filterOption.display_size.filter((e,i) => selectedDisplaySize[i]),
      searchValue
    }
      ...
      <Container maxWidth="xl">
        <div className="home">
          <div className="filter">
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  label="Tìm kiếm"
                  value={searchValue}
                  onChange={(e) => {dispatch(updateState({searchValue: e.target.value}))}}
                  InputProps={{
                    ...params.InputProps,
                    type: "search",
                    endAdornment: (
                    ...
                    filterOption.ram.map((e, i) => {
                      if (i % 2 === 0) {
                        return (
                          <FormControlLabel
                            key={i}
                            control={<Checkbox checked={selectedRam[i]} onChange={() => {
                    ...
                    filterOption.rom.map((e, i) => {
                      if (i % 2 === 0) {
                        return (
                          <FormControlLabel
                            key={i}
                            control={<Checkbox checked={selectedRom[i]} onChange={() => {
                    ...
                    filterOption.battery.map((e, i) => {
                      if (i % 2 === 0) {
                        return (
                          <FormControlLabel
                            key={i}
                            control={<Checkbox checked={selectedBattery[i]} onChange={() => {
                              const temp = [...selectedBattery]
                              temp[i] = !temp[i];
                              dispatch(updateState({selectedBattery: temp}))
                            }}/>}
                            label={`${e}mAh`}
                          />
                        );
            ...
            <div>
              <div className="button">
                <Button variant="contained" onClick={search}>Áp dụng</Button>
                <Button className="button-cancel" variant="outlined" onClick={() => {
                  dispatch(updateState({
                    selectedDisplaySize: selectedDisplaySize.map(()=> false),
          ...
          <div className="right-page">
            ...
            <div className="page">
              <Pagination count={totalPage} onChange={(e ,v) => {dispatch(updateState({page: v}))}} color="primary" />
            </div>
          </div>
        </div>
      </Container>
